...
const SidebarLayout = (): JSX.Element => {
  ...
  const handleSlideSidebar = (): void => {
    ...
    if (
      asideRef.current?.classList.contains("h-[235px]") ||
      asideRef.current?.classList.contains("h-[400px]")
    ) {
      asideRef.current?.classList.toggle("h-[235px]");

      asideRef.current?.classList.toggle("h-[400px]");
    }
  ...
  // ${
  //   pathname == "/ListEventPage" ? "h-[500px]" : "h-56"
  // }
  ...
  return (
    <>
      <aside
        ref={asideRef}
        className={`relative lg:static overflow-hidden bg-[#f5f5f5] rounded-2xl lg:h-[90dvh] transition-all duration-300 h-[235px]`}
      >
        ...
        >
          ...
          <ul
            ref={slideSidebarRef}
            className="h-fit lg:static lg:translate-x-0 translate-x-52 absolute top-20 right-5 w-fit capitalize font-semibold text-slate-700 mt-2 2xl:mt-10 text-lg flex flex-col justify-start items-start gap-5 2xl:gap-6 lg:w-[90%] transition-all duration-300"
          >
            <li className="transition-all duration-150 hover:text-slate-800 text-sm 2xl:text-md">
              <Link
                onClick={() => {
                  if (pathname == "/") {
                    asideRef.current?.classList.replace(
                      "h-[400px]",
                      "h-[235px]"
                    );
                  }
            ...
            <li>
              <Link
                className="lg:static absolute flex items-center justify-center gap-3 px-4 py-2 2xl:px-8 2xl:py-3 mt-auto capitalize transition-all duration-150 rounded-md shadow-md text-sm 2xl:text-md bg-slate-100 text-slate-900 hover:bg-slate-600 hover:text-slate-100"
                href={"/"}
              >
                <LogoutIcon />
                logout
